...
class MxDeepFake(mx.Disposable):
    ...
    def __init__(self, open_path : Path|None = None):
        ...
        self._mx_gen_src : MxDataGenerator = None
        self._mx_gen_dst : MxDataGenerator = None

        self._mx_model : MxModel = None
        self._mx_model_trainer : MxModelTrainer = None
        self._mx_preview : MxPreview = None

        file_state_mgr = self._mx_file_state_mgr = MxFileStateManager( file_suffix='.dxf',
    ...
    @property
    def mx_gen_src(self) -> MxDataGenerator:
        """avail only when .file_state_manager.mx_state is Initialized """
        return self._mx_gen_src

    @property
    def mx_gen_dst(self) -> MxDataGenerator:
        """avail only when .file_state_manager.mx_state is Initialized """
        return self._mx_gen_dst
    ...
    def _on_load(self, state : dict):
        ...
        self._mx_gen_src = MxDataGenerator(state=state.get('gen_src', None), default_rnd_flip=False).dispose_with(disp_bag)
        self._mx_gen_dst = MxDataGenerator(state=state.get('gen_dst', None), default_rnd_flip=True).dispose_with(disp_bag)

        self._mx_model         = MxModel( state=state.get('model', None)).dispose_with(disp_bag)
        self._mx_model_trainer = MxModelTrainer(self._mx_gen_src, self._mx_gen_dst, self._mx_model, state=state.get('model_trainer', None)).dispose_with(disp_bag)
        self._mx_preview       = MxPreview(self._mx_gen_src, self._mx_gen_dst, self._mx_model, state=state.get('preview', None)).dispose_with(disp_bag)
    ...
    def _get_state(self) -> dict:
        ...
        return {'gen_src' : self._mx_gen_src.get_state(),
                'gen_dst' : self._mx_gen_dst.get_state(),
